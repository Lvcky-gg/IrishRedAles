...
const ReveiwCard = ({ id, User, rating, description }) => {
  ...
  let userString;
  if (User)
    userString = `${User.firstName} ${User.lastName[0].toUpperCase()}. Says...`;
  ...
  useEffect(() => {
    if (sessionUser) {
      ...
      for (let i = 0; i < reviewLikes.length; i++) {
        if (
          reviewLikes[i].userId === userId &&
          reviewLikes[i].reviewId === id
        ) {
          setIsLiked(true);
        }
      }
  ...
  const onDeleteLike = (e) => {
    ...
    for (let i = 0; i < reviewLikes.length; i++) {
      if (reviewLikes[i].userId === userId && reviewLikes[i].reviewId === id) {
        dispatch(
          deleteReviewLike({ reviewId: +id, likeId: reviewLikes[i].id })
        );
      }
    }
    setIsLiked(false);
